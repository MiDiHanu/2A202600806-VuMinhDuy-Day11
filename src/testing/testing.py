...
async def run_comparison():
    ...
    # --- Protected agent ---
    # Make sure the LLM judge agent exists before constructing the
    # output guardrail (it skips itself if the judge isn't initialized).
    _init_judge()
    input_plugin = InputGuardrailPlugin()
    output_plugin = OutputGuardrailPlugin(use_llm_judge=False)  # keep fast for the test sweep
